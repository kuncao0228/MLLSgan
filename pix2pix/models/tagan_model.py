...
class TAGAN_G(nn.Module):
    def __init__(self):
        super(TAGAN_G, self).__init__()
    ...
    def forward(self, img, txt):
        ...
        # print(torch.cat((e, cond.repeat(1, 1, e.size(2), e.size(3))), 1).size())
...
# text = torch.randn(5, 4, 300).cuda() #vocab size x batch x embed size
# len_txt = torch.ones(4).cuda()

# G = TAGAN_G()
# G = G.cuda()

# x, y = G(img , (text, len_txt))
