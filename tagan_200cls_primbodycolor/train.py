    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (img, txt, len_txt) in enumerate(train_loader):
            ...
            if i % args.log_interval == 0:
                message = 'Epoch [%03d/%03d], Iter [%03d/%03d], D_real: %.4f, D_real_c: %.4f, D_fake: %.4f, G_fake: %.4f, G_fake_c: %.4f, G_recon: %.4f, KLD: %.4f' % (epoch + 1, args.num_epochs, i + 1, len(train_loader), avg_D_real_loss / (i + 1), avg_D_real_c_loss / (i + 1), avg_D_fake_loss / (i + 1), avg_G_fake_loss / (i + 1), avg_G_fake_c_loss / (i + 1), avg_G_recon_loss / (i + 1), avg_kld / (i + 1))
                print(message)  # print the message
                with open('taganloss_log.txt', "a") as log_file:
